...
if __name__ == "__main__":
    ...
    if "celeb" in DATASET:
        ...
        metadata_store = np.load(f"{PATH}/metadata_m=5.npz")['metadata']
    ...
    idx = 0
    valid_query_idx = []
    for query in tqdm.tqdm(queries, desc="Computing ground truth", unit="query"):
        ground_truth_ilp(
            query,
            vector_store,
            metadata_store,
        ...
        if query is not None and 'ground_truth' in query:
            count += 1
    ...
    with open(f'{PATH}/queries_k={args.k}_m={args.m}^5_fdist={args.fdist}_{len(queries)}.pkl', 'wb') as f:

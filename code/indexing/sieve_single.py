...
class SIEVESingle:
    ...
    def __init__(
        ...
        self.args = args
        self._dim = vector_store[0].shape[-1]
    ...
    # ------------------------- Fitting & Building -------------------------
    def build_subindexes(self) -> None:
        for key, ids_list in self.partitions.items():
            ...
            vecs = self.vector_store[ids_arr]
            # Choose backend
    ...
    def search_and_solve(
        ...
        query_vec = query['text_query_embedding']
        k = int(query['k'])
        queried_parts = [
        ...
        start = time.time()
        for key in queried_parts:
            attr, val = key.split(":", 1)
            requirement = constraints[attr][val]
